...
const page = {
	...
	"seriesStatsOrderButton": document.getElementById("series-stats-order-button"),
	"seriesDurations": document.getElementById("series-durations"),
	"seriesShowCounts": document.getElementById("series-show-counts"),
...
function buildPlaylistShowHTML(show) {
	...
	HTML += '<ul class="show-position-controls">' +
...
function revealClearPlaylistControls() {
	...
	page.clearPlaylistControls.classList.remove("hidden");
...
function hideClearPlaylistControls() {
	...
	page.clearPlaylistControls.classList.add("hidden");
}
...
// switch series stats sub-list between ordered by duration and ordered by show counts
function reorderSeriesStats() {
	if (page.seriesStatsOrderButton.dataset.order === "duration") {
...
// count series and shows in archive and hours of audio total and per-series, then order series by duration
function countStats() {
	const seriesStats = [];
	let totalCount = 0,
	seriesDurationsHTML = "",
	seriesShowCountsHTML = "",
	totalDuration = 0;
	...
	for (const series of archive) {
		...
		for (const show of series.shows) seriesDuration += show.duration;
		totalDuration += seriesDuration;
		seriesStats.push({
			"heading": series.heading.replace(/ title=".+"/g, ""),
			"count": series.shows.length,
			"duration": seriesDuration
		});
	}

	// reorder by show counts first, because the initial order is alphabetical and it's very unlikely for 2+ series to share total durations
	seriesStats.sort((a, b) => (b.count - a.count));
	for (const series of seriesStats) {
		seriesShowCountsHTML += "<dt>" + series.heading + "</dt><dd>" + series.count + " shows</dd>";
	}

	seriesStats.sort((a, b) => (b.duration - a.duration));
	for (const series of seriesStats) {
		seriesDurationsHTML += "<dt>" + series.heading + "</dt><dd>" + (Math.round(series.duration / 3600 * 10) / 10).toFixed(1) + " hours</dd>";
	}

	document.getElementById("loading-spinner-stats")?.remove();
	document.getElementById("archive-stats-list").innerHTML = "<div><dt>sources</dt><dd>" + archive.length + "</dd></div>" +
"<div><dt>shows</dt><dd>" + totalCount + "</dd></div>" +
"<div><dt>hours of audio</dt><dd>" + Math.round(totalDuration / 3600) + "</dd></div>";

	document.getElementById("loading-spinner-show-stats")?.remove();
	page.seriesDurations.innerHTML = seriesDurationsHTML;
	page.seriesShowCounts.innerHTML = seriesShowCountsHTML;
	document.getElementById("series-stats-content").classList.remove("hidden");
}
